...
ReceiverTactic::ReceiverTactic(const Field& field, const Team& friendly_team, const Team& enemy_team, const AI::Passing::Pass pass, const Ball& ball, bool loop_forever)
    : field(field), pass(std::move(pass)), ball(ball), friendly_team(friendly_team),
    enemy_team(enemy_team), Tactic(loop_forever)
{
}
...
void ReceiverTactic::updateParams(const Team& updated_friendly_team, const Team& updated_enemy_team,
    ...
    intent_coroutine::push_type& yield)
{
    ...
    // possible
        ...
        while((ball.position() - robot->position()).len() > DIST_TO_FRONT_OF_ROBOT_METERS + 0.03) {
