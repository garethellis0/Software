...
void CanvasMessenger::publishAllLayers()
{
...
{
    // TODO: We should probably just have layers for each item, and then each function
    // should clear the layer for that item (ball, team, field, etc.)
    // Clear previous work
    clearLayer(Layer::STATIC_FEATURES);
    clearLayer(Layer::ROBOTS_AND_BALL);
