...
2. From the `src` folder, run `bazel run --cpu=stm32h7 --crosstool_top=//cc_toolchain:toolchain --compilation_mode=dbg //firmware_new/tools:debug_firmware_on_arm_board`. We specify `--cpu=stm32h7`. These flags indicate that we want to compile code for the stm32h7 MCU (rather then a `x86_64` processor like you have in your computer), use our custome toolchain to build the firmware, and build in the debug symbols required so you can step through the code and see what's going on. You'll be given a list of elf files to choose from.
3. Assuming you choose 0 from the list in step (2), run `bazel run --cpu=stm32h7 --crosstool_top=//cc_toolchain:toolchain --compilation_mode=dbg //firmware_new/tools:debug_firmware_on_arm_board 0`. This will load the `.elf` file associated with (0) to the the nucleo and put you into a gdb prompt.
